...
//		ret->setEventData(edata);
	...
	* defined by two points p1 and p2, gets measured against the normaldistribution that
	* is within this cell.
	...
	*/
	//
	//   计算一个点沿着由点p1和p2定义的直线移动时，正好落入位于本NDT单元内的正态分布中的最大似然度。
	//   这主要用于扫描线跟踪分析，用来判断一条扫描线是否穿透一个先前测量到的物体，从而判断出得到了一个不一致的证据。
	//
	//   p1, p2: 扫描线上的两个点
	//   out:    p1-p2之间落入正态分中具有最大似然度的的那个对应点
	//
...
#define _RGB(r,g,b)          ((COLORREF)(((BYTE)(r)|((WORD)((BYTE)(g))<<8))|(((DWORD)(BYTE)(b))<<16)))
		...
		bool bShowMatched, unsigned long clrMatched)
	{
		if (hasGaussian_ && (occ > 0))
		{
			...
			// 如果需要差异化显示已匹配的项
			if (bShowMatched && matchStatus == 1)
				clrLine = clrFill = clrMatched;

			ell.Draw(ScrnRef, pDc, clrLine, 1, clrFill, true);
		}
	}
#elif defined QT_VERSION
	void NDTCell::Plot(CScreenReference& ScrnRef, QPainter* pPainter, QColor clrLine, QColor clrFill,
...
///////////////////////////////////////////////////////////////////////////////
		...
		bool bShowMatched, unsigned long clrMatched)
	{
		...
		{
			at(i)->Plot(ScrnRef, pDc, clrLine, clrFill, bShowMatched, clrMatched);
		}
	}

#elif defined QT_VERSION
void VectNDTCells::Plot(CScreenReference& ScrnRef, QPainter* pPainter, QColor clrLine, QColor clrFill,
	bool bShowMatched, QColor clrMatched)
{
	for (unsigned int i = 0; i < size(); i++)
	{
		at(i)->Plot(ScrnRef, pPainter, clrLine, clrFill, bShowMatched, clrMatched);
	}
}
